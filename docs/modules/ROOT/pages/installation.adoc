...
There are three ways to run the Monitoring Operator:
...
----
sudo apt-get update
sudo apt-get install stackable-monitoring-operator
----
...
[source,bash]
----
sudo yum install stackable-monitoring-operator
----
...
[source]
----
docker.stackable.tech/stackable/monitoring-operator
----

To run it straight from Docker you can use this command:
[source,bash]
----
docker run -e MONITORING_OPERATOR_LOG=trace -e KUBECONFIG=/root/.kube/k3s.yaml --mount type=bind,source="$HOME/.kube/k3s.yaml",target="/root/.kube/k3s.yaml" -it docker.stackable.tech/stackable/monitoring-operator:latest
----
